...
struct TextExchangeOption
{
    ...
    OSString origin_path;
    OSString origin_file;

    OSString replace_path;
    OSString replace_file;
...
{
    std::cout<<"CMGDK tools << Text Exchange v1.0 >>"<<std::endl;
    ...
    if(args.GetCount()<11)
    {
        ...
        std::cout<<std::endl;
        std::cout<<" Format: TextExchange [-cc,-ce,-cs,-cb,-sf] <shift_jis|big5|gbk|...> <input path> <input filename> <orign path> <orign file> <replace path> <replace file> <output path> <output file extname>"<<std::endl;
        std::cout<<" Example: TextExchange -cc -ce -cs -cb -sf shift_jis /home/hyzboy/input_csv/ .csv /home/hyzboy/orign_path/ .txt /home/hyzboy/replace_path/ .txt /home/hyzboy/output_txt/ txt"<<std::endl;
        return(0);
    }

    TextExchangeOption teo;
    ...
    teo.origin_path=args[off++];
    teo.origin_file=args[off++];
    teo.replace_path=args[off++];
    teo.replace_file=args[off++];
    ...
    std::cout<<"Origin Path: "<<teo.origin_path.c_str()<<std::endl;
    std::cout<<"Origin Filename: "<<teo.origin_file.c_str()<<std::endl;
    std::cout<<"Replace Path: "<<teo.replace_path.c_str()<<std::endl;
    std::cout<<"Replace Filename: "<<teo.replace_file.c_str()<<std::endl;
