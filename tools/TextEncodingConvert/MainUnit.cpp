...
class TECEnumFile:public EnumFile
{
public:

    EnumFileConfig *CreateSubConfig(struct EnumFileConfig *efc,const FileInfo &fi) override
    {
        ...
        return EnumFile::CreateSubConfig(efc,fi);
    }
    ...
    void ProcFile(struct EnumFileConfig *efc,FileInfo &fi) override
    {
        if(fi.name[0]=='.')return;
        ...
        const os_char *ext_name=strrchr<os_char>(fi.name,strlen(fi.name),'.');
...
HGL_CONSOLE_MAIN_FUNC()
{
    ...
    std::cout<<"(C)1997-2017 Copyright. Offical Web:www.hyzgame.com"<<std::endl;
    std::cout<<std::endl;
    ...
    if(args.GetCount()<5)
    {
        ...
        std::cout<<" Example: TextEncodingConvert -in us-ascii /home/hyzboy/input/ -out utf8 /home/hyzboy/output/"<<std::endl;
        std::cout<<std::endl;
