...
namespace hgl
{
	...
	bool List<T>::Get(int index,T &ti)const
	{
		...
		{
            LOG_ERROR(OS_TEXT("List<>::Get(index=")+OSString::valueOf(index)+OS_TEXT(") error,DataCount=")+OSString::valueOf(count));
			return(false);
		}
	...
	T &List<T>::operator[](int index)const															///<操作符重载取得指定索引处的数据
	{
		if(!items||index<0||index>=count)
		{
            ...
            LOG_ERROR(OS_TEXT("List<>::operator(index=")+OSString::valueOf(index)+OS_TEXT(") error,DataCount=")+OSString::valueOf(count));
	...
	void List<T>::Set(int index,const T &val)
	{
			...
			if(!items||index<0||index>=count)
			{
                LOG_ERROR(OS_TEXT("List<>::Set(index=")+OSString::valueOf(index)+OS_TEXT(",T &) error,DataCount=")+OSString::valueOf(count));
			}
			else
