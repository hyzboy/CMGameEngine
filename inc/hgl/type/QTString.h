...
#include<QString>
    ...
    inline UTF8String ToUTF8String(const QString &str)
    {
        QByteArray u8str=str.toUtf8();
        
        return UTF8String(u8str.data(),u8str.size());
    }

    inline WideString ToOSString(const QString &str)
    {
        //qt有toWCharArray(),但是不能预估wchar_t的长度，所以暂时使用先UTF8，再转utf16的方式
        
        QByteArray u8str=str.toUtf8();
        
        return to_u16(u8str.data(),u8str.size());
    }
#else
    ...
    inline UTF8String ToOSString(const QString &str)
    {
