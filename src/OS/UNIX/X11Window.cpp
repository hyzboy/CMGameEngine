...
namespace hgl
{
	...
	bool X11Window::CreateWindow(const char *win_name,int w,int h)
	{
		if(disp)
			return(false);
	...
	bool X11Window::InitToFullScreen(int w, int h, int c,bool tm)
	{
		if(!CreateWindow("CMGDK",w,h))
			return(false);
	...
	bool X11Window::InitToWindow(int w,int h,GraphicsSystemInitInfo::WindowSetup &ws)
	{
		const UTF8String win_name=to_u8(ws.Name);

		return CreateWindow((char *)(win_name.c_str()),w,h);
	}
