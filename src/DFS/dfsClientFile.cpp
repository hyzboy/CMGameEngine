...
namespace hgl
{
	namespace dfs
	{
		 ...
		 * 初始化dfs文件客户端
		 * @param _ip dfs服务器ip
		 * @param _port dfs服务器端口
		 * @param _node_name 当前节点名称
		 * @param _node_id 当前节点id
		 * @return 是否初始化成功
		 */
		bool dfsClientFile::Init(const UTF8String &_ip,uint _port,const UTF8String &_node_name,const int64 _node_id)
		{
			if(!dfsClientConnect::Init(_ip,_port,_node_name,_node_id))
				RETURN_FALSE;

			conn_pool=new dfsClientFileConnectPool(set_name,_ip,_port,_node_name,_node_id);

			if(!Use())
				RETURN_FALSE;

			save_thread=new dfsClientSaveThread(set_name,_ip,_port,node_id);
